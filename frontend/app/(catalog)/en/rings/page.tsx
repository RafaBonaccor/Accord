...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  return buildCatalogMetadata({
    title: "Rings",
    description: "Discover Accordi Jewelry rings with luminous finishes, refined details and gift-ready styles.",
    locale: "en",
    currentPage: page,
    basePath: "/en/rings",
    alternateBasePath: "/anelli",
  });
}

export default async function RingsPage({ searchParams }: Props) {
  const page = parsePageParam((await searchParams).page);
  ...
  const copy = categoryPageCopy.en.rings;
  ...
  return (
    <CatalogPage
      ...
      pageHref={(nextPage) => pagePath("/en/rings", nextPage)}
    />
  );
}
